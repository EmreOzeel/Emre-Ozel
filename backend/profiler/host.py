...
def build_profiles(ctx: CaptureContext) -> None:
    ...
    # ── Pass 1: compute unique port counts FIRST (needed for anomaly scoring) ──
    ...
    # ── Pass 2: finalize all per-host fields ─────────────────────────────────
    for ip, profile in profiles.items():
        ...
        initiated = profile.tcp_sessions_initiated
        failed = profile.tcp_sessions_failed
        if initiated > 0:
            profile.connection_success_ratio = max(0.0, (initiated - failed) / initiated)

        # Protocol mix percentages
        total_proto_frames = sum(profile.protocols.values())
        if total_proto_frames > 0:
            profile.protocol_mix_pct = {
                k: round(v / total_proto_frames * 100, 1)
                for k, v in sorted(profile.protocols.items(), key=lambda x: -x[1])
            }
        ...
        # Periodic external connection = strong anomaly (beaconing from internal host)
        if interval > 0 and _is_private(ip):
            score += 3.0
            behaviors.append(f"Periodic connections every ~{interval:.0f}s (jitter={jitter:.2f})")

        # Scanning: many unique ports contacted (now computed before this block)
        if profile.unique_dst_ports > 50:
            score += 3.0
            behaviors.append(f"Contacted {profile.unique_dst_ports} unique destination ports")

        # High failure rate
        total_initiated = profile.tcp_sessions_initiated + profile.tcp_sessions_failed
        if total_initiated > 10:
            fail_rate = profile.tcp_sessions_failed / total_initiated
            if fail_rate > 0.5:
                score += 2.0
    ...
    # ── Pass 3: populate peer_roles now that all profiles + roles are known ──
    for ip, profile in profiles.items():
        profile.peer_roles = {
            peer_ip: profiles[peer_ip].role.value
            for peer_ip, _ in profile.top_peers
            if peer_ip in profiles
        }
